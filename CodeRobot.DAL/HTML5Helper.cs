...
namespace CodeRobot.DAL
{
    public class HTML5Helper
    {
        ...
        public static void CreateHTML5File(string strFilePath, string strProjectName, string strTableName, string strTableComment)
        {
            ...
            sw.WriteLine("");
            sw.WriteLine(SetRequestParas(strTableName));//参数
            sw.WriteLine("            try");
            sw.WriteLine("            {");
            sw.WriteLine("                /*string strImgPath = " + strProjectName + ".Utility.CommonHelper.GetCookieString(\"" + strProjectName + "_Admin_UploadFileName\", true);");
            sw.WriteLine("                if (string.IsNullOrEmpty(strImgPath))");
            sw.WriteLine("                {");
            sw.WriteLine("                    strImgPath = \"\";");
            sw.WriteLine("                }");
            sw.WriteLine("                strNewsContent = System.Web.HttpUtility.UrlDecode(strNewsContent);*/");
            ...
            sw.WriteLine("");
            sw.WriteLine("                strReturnValue = \"{\\\"code\\\":\\\"0\\\",\\\"msg\\\":\\\"添加成功\\\"}\"; ");
            sw.WriteLine("            }");
            ...
            sw.WriteLine("                strReturnValue = \"{\\\"code\\\":\\\"1\\\",\\\"msg\\\":\\\"添加失败\\\"}\"; ");
            sw.WriteLine("            }");
            sw.WriteLine("");
            ...
            sw3.WriteLine("            try");
            sw3.WriteLine("            {");
            sw3.WriteLine("                " + strProjectName + ".BLL." + strClassNameUpper + "BLL.DeleteData(\" WHERE " + strPrimaryID + "=\" + str" + GetPrimaryKey(strTableName, true) + ");");
            sw3.WriteLine("");
            ...
            sw5.WriteLine("");
            sw5.WriteLine(SetRequestParas(strTableName, 2));//参数
            sw5.WriteLine("            try");
            sw5.WriteLine("            {");
            sw5.WriteLine("                /*string strImgPath = " + strProjectName + ".Utility.CommonHelper.GetCookieString(\"" + strProjectName + "_Admin_UploadFileName\", true);");
            sw5.WriteLine("                if (string.IsNullOrEmpty(strImgPath))");
            sw5.WriteLine("                {");
            sw5.WriteLine("                    strImgPath = \"\";");
            sw5.WriteLine("                }");
            sw5.WriteLine("                strNewsContent = System.Web.HttpUtility.UrlDecode(strNewsContent);*/");
            ...
            sw5.WriteLine("");
            sw5.WriteLine("                strReturnValue = \"{\\\"code\\\":\\\"0\\\",\\\"msg\\\":\\\"修改成功\\\"}\"; ");
            sw5.WriteLine("            }");
            ...
            sw5.WriteLine("                strReturnValue = \"{\\\"code\\\":\\\"1\\\",\\\"msg\\\":\\\"修改失败\\\"}\"; ");
            sw5.WriteLine("            }");
            sw5.WriteLine("");
            ...
            //显示页面路径
            string strViewFilePath = strFilePath + "\\" + strClassName + "\\view";
            if (!Directory.Exists(strViewFilePath))
            {
                Directory.CreateDirectory(strViewFilePath);
            }
            ...
            sw7.WriteLine("            try");
            sw7.WriteLine("            {");
            sw7.WriteLine("                List<" + strProjectName + ".Model." + strClassNameUpper + "Model> list = " + strProjectName + ".BLL." + strClassNameUpper + "BLL.SelectAllByWhere(\"\");");
            sw7.WriteLine("                foreach(" + strProjectName + ".Model." + strClassNameUpper + "Model model in list)");
            sw7.WriteLine("                {");
            sw7.WriteLine("");
            sw7.WriteLine(GetDataListForListPage(strTableName));//传入参数
            sw7.WriteLine("");
            sw7.WriteLine("                }");
            sw7.WriteLine("");
            sw7.WriteLine("                if (list.Count == 0)");
            sw7.WriteLine("                {");
            sw7.WriteLine("                    strShowCSS = \"style=\\\"margin: 50px 0; text - align: center; display: block;\\\"\";");
            sw7.WriteLine("                }");
            sw7.WriteLine("                else");
            sw7.WriteLine("                {");
            sw7.WriteLine("                    strShowCSS = \"style=\\\"margin: 50px 0; text - align: center; display: none;\\\"\";");
            sw7.WriteLine("                }");
            sw7.WriteLine("            }");
            ...
            //生成Default.aspx文件
            //读取模板内容
            FileStream fs1 = new FileStream(Application.StartupPath + "\\template\\html\\h5_list.html", FileMode.OpenOrCreate, FileAccess.Read);
            StreamReader sr1 = new StreamReader(fs1, Encoding.UTF8);
            string strGetPageHTMLContentForManagePage = sr1.ReadToEnd();
            sr1.Close();
            fs1.Close();

            strGetPageHTMLContentForManagePage = GetHTMLDataForListPage(strGetPageHTMLContentForManagePage, strTableName);
            ...
            //生成add.aspx文件-未实现
            //读取模板内容
            FileStream fs2 = new FileStream(Application.StartupPath + "\\template\\html\\h5_add.html", FileMode.OpenOrCreate, FileAccess.Read);
            ...
            //生成edit.aspx文件-未实现
            //读取模板内容
            FileStream fs3 = new FileStream(Application.StartupPath + "\\template\\html\\h5_edit.html", FileMode.OpenOrCreate, FileAccess.Read);
            ...
            //生成view/default.aspx
            //读取模板内容-未完成
            FileStream fs4 = new FileStream(Application.StartupPath + "\\template\\html\\h5_view.html", FileMode.OpenOrCreate, FileAccess.Read);
            StreamReader sr4 = new StreamReader(fs4, Encoding.UTF8);
            string strGetPageHTMLContentForViewPage = sr4.ReadToEnd();
            sr4.Close();
            fs4.Close();

            strGetPageHTMLContentForViewPage = GetEditPageHTMLData(strGetPageHTMLContentForViewPage, strTableName, strManageFilePath, strProjectName);
            StreamWriter sw41 = new StreamWriter(strViewFilePath + "\\Default.aspx", false, Encoding.GetEncoding("utf-8"));
            sw41.WriteLine(strGetPageHTMLContentForEditPage);
            sw41.Close();
            ...
            //搜索页面-------------------------------------------7
            //读取模板内容
            FileStream fs12 = new FileStream(Application.StartupPath + "\\template\\html\\h5_search.html", FileMode.OpenOrCreate, FileAccess.Read);
        ...
        /// <param name="strTableName">表名</param>
        /// <returns></returns>
        public static string SetRequestParas(string strTableName)
        {
        ...
        /// <returns></returns>
        public static string SetRequestParas(string strTableName, int nFlag)
        {
            string strReturnValue = "";
        ...
        /// <param name="strTableName">表名</param>
        /// <returns></returns>
        public static string GetDataListForListPage(string strTableName)
        {
            string strReturnValue = "";
            ...
            try
            {
                ...
                string strDataValue = "                    string strColumn = \"<li>\";\r\n";
                strDataValue += "                    strColumn += \"<a href=\\\"/?id=\" + "+ strPrimaryKeyValue + " + \"\\\" class=\\\"item-link item-content\\\" external>\";\r\n";
                strDataValue += "                    strColumn += \"<div class=\\\"item-inner\\\">\";\r\n";
                strDataValue += "                    strColumn += \"<div class=\\\"item-title-row\\\">\";\r\n";
                strDataValue += "                    strColumn += \"<div class=\\\"item-title\\\"><strong>测试标题</strong></div>\";\r\n";
                strDataValue += "                    strColumn += \"<div class=\\\"item-after\\\">03-04 15:40</div>\";\r\n";
                strDataValue += "                    strColumn += \"</div>\";\r\n";
                strDataValue += "                    strColumn += \"<div class=\\\"item-subtitle\\\">状态</div>\";\r\n";
                strDataValue += "                    strColumn += \"<div class=\\\"item-text\\\">摘要内容摘要内容摘要内容摘要内容摘要内容摘要内容摘要内容摘要内容</div>\";\r\n";
                strDataValue += "                    strColumn += \"</div>\";\r\n";
                strDataValue += "                    strColumn += \"</a>\";\r\n";
                strDataValue += "                    strColumn += \"</li>\";\r\n";
                strDataValue += "                    strShowDataList += strColumn;\r\n";
        ...
        /// <param name="strTableName">表名</param>
        /// <returns></returns>
        public static string GetHTMLDataForListPage(string strContent, string strTableName)
        {
            string strReturnValue = "";
            ...
            try
            {
                ...
                strContent = strContent.Replace("{classname}", strClassName);
                //替换2：表名
                strContent = strContent.Replace("{tablename}", strTableNameComment);

                strReturnValue = strContent;
            }
            catch (Exception ex)
            {
                CodeRobot.Utility.LogHelper.Error(typeof(ManageHelper), ex, "获取aspx页面HTML代码", "GetHTMLDataForListPage",false);
            }
                ...
                while (dr.Read())
                {
                    ...
                    if (strColumnKey != "PRI")
                    {
                        ...
                        if (strColumnComment.Contains("{下拉}"))
                        {
                            ...
                            strColumn += "                                            <select class=\"form-control input-lg m-bot15\" id=\"" + strColumnName + "\">\r\n";
                        ...
                        else
                        {
                            ...
                            strColumn += "                                        <label for=\"cname\" class=\"control-label col-lg-2\">" + strColumnComment2 + " <!--<span class=\"color-red\">(*)</span>--></label>\r\n";
                            strColumn += "                                        <div class=\"col-lg-4\">\r\n";
                            strColumn += "                                            <input class=\"form-control\" type=\"text\" id=\"" + strColumnName + "\" name=\"" + strColumnName + "\" />\r\n";
                            strColumn += "                                        </div>\r\n";
                            strColumn += "                                    </div>\r\n";
                        }
                ...
                while (dr.Read())
                {
                    ...
                    if (strColumnKey != "PRI")
                    {
                        ...
                        else
                        {
                            ...
                            strColumn += "                                        <label for=\"cname\" class=\"control-label col-lg-2\">" + strColumnComment2 + " <!--<span class=\"color-red\">(*)</span>--></label>\r\n";
                            strColumn += "                                        <div class=\"col-lg-4\">\r\n";
                            strColumn += "                                            <input class=\"form-control\" type=\"text\" id=\"" + strColumnName + "\" name=\"" + strColumnName + "\" value=\"<%=strShow" + strName + " %>\" />\r\n";
                            strColumn += "                                        </div>\r\n";
                            strColumn += "                                    </div>\r\n";
                        }
        ...
        public static string GetSearchPageHTMLData(string strContent, string strTableName, string strManageFilePath, string strProjectName)
        {
            ...
            try
            {
                    ...
                    if (strColumnKey != "PRI")
                    {
                        ...
                        if (strColumnComment.Contains("{搜索}"))
                        {
                            ...
                            else if (strColumnComment.Contains("{下拉}"))
                            {
                                ...
                                {
                                    string strColumnSelect = "\r\n                //" + strGetUnionTableName + " default\r\n";
                                    strColumnSelect += "                List<" + strProjectName + ".Model." + strGetUnionTableName + "Model> list" + nSelectNum + " = " + strProjectName + ".BLL." + strGetUnionTableName + "BLL.SelectAllByWhere(\"WHERE parent_id=-1\");\r\n";
                                    strColumnSelect += "                foreach(" + strProjectName + ".Model." + strGetUnionTableName + "Model data in list" + nSelectNum + ")\r\n";
                                    strColumnSelect += "                {\r\n";
                                    strColumnSelect += GetDataListForEditPage(strGetSelectTableName);//传入参数
                                    strColumnSelect += "                }\r\n";
                            ...
                            else
                            {
                                //UI
                                strColumn = "\r\n                                                <div class=\"col-lg-4\">\r\n";
                                strColumn += "                                                    " + strColumnComment2 + "：<input type=\"text\" class=\"form-control\" id=\"" + strColumnName + "\" name=\"" + strColumnName + "\" placeholder=\"输入关键词\" value=\"<%=strShow" + strName + " %> \">\r\n";
                                strColumn += "                                                </div>\r\n";
                ...
                strColumnCodeStartDate += "                strShowStartDate = DateTime.Now.ToString(\"yyyy-MM-dd\");\r\n";
                strColumnCodeStartDate += "            }\r\n";
                ...
                strColumnCodeStartDate += "                strShowStartDate = strStartDate;\r\n";
                strColumnCodeStartDate += "            }\r\n";
                //结束日期
                ...
                strColumnCodeEndDate += "                strShowEndDate = DateTime.Now.AddDays(1).ToString(\"yyyy-MM-dd\");\r\n";
                ...
                strColumnCodeEndDate += "                strShowEndDate = strEndDate;\r\n";
                ...
                {
                    strSearchColumnParas = strSearchColumnParas.Substring(0, strSearchColumnParas.Length - 1);
                }

                if (!string.IsNullOrEmpty(strSearchColumnParas))
                {
                    strWhereParas = "\r\n            string strWhere = \"WHERE created_at>='\" + strStartDate + \"'  and created_at<= '\" + strEndDate + \"' \" + " + strSearchColumnParas + ";\r\n";
                }
                else
                {
                    strWhereParas = "\r\n            string strWhere = \"WHERE created_at>='\" + strStartDate + \"'  and created_at<= '\" + strEndDate + \"' \";\r\n";
                }
                ...
                sw.WriteLine("            {");
                sw.WriteLine("");
                sw.WriteLine(GetDataListForListPage(strTableName));//传入参数
                sw.WriteLine("");
                sw.WriteLine("             }");
                sw.WriteLine("         }");
                sw.WriteLine("        catch (Exception ex)");
                sw.WriteLine("        {");
                sw.WriteLine("            " + strProjectName + ".Utility.LogHelper.Error(typeof(" + strClassName + "_Search), ex, \"\", \"Page_Load\", true);",false);
                sw.WriteLine("        }");
                sw.WriteLine("");
                sw.WriteLine("    }");
                sw.WriteLine("}");
