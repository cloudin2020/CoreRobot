...
namespace CodeRobot.DAL
{
    ...
    public class ContextHelper
    {
        ...
        public static void CreateContextClass(string strFilePath,string strProjectName, string strContext)
        {
            ...
            StreamWriter sw = new StreamWriter(strFilePath + "\\" + strProjectName + "ManageContext.cs", false, Encoding.GetEncoding("utf-8"));
            sw.WriteLine("using System;");
            sw.WriteLine("using Microsoft.EntityFrameworkCore;");
            sw.WriteLine("");
            sw.WriteLine("namespace " + CommonHelper.GetTableNameUpper(strProjectName) + ".Manage.Data");
            sw.WriteLine("{");
            sw.WriteLine("");
            sw.WriteLine("    /// <summary>");
            sw.WriteLine("    /// 版权所有: Copyright © " + DateTime.Now.Year.ToString() + " "+strCompany+". 保留所有权利。");
            sw.WriteLine("    /// 内容摘要: " + strProjectName + "ManageContext 数据库上下文类,EF Core ");
            ...
            sw.WriteLine("    /// </summary>");
            sw.WriteLine("    public class " + strProjectName + "ManageContext : DbContext");
            sw.WriteLine("    {");
            sw.WriteLine("        /// <summary>");
            sw.WriteLine("        /// 数据库上下文");
            sw.WriteLine("        /// </summary>");
            sw.WriteLine("        public " + strProjectName + "ManageContext(DbContextOptions<" + strProjectName + "ManageContext> options): base(options)");
            sw.WriteLine("        {");
            sw.WriteLine("        }");
            sw.WriteLine("        ");
